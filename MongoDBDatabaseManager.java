...
public class MongoDBDatabaseManager implements DatabaseManagerStore{
	...
	public Collection<WarrantyDeed> getAll() {
		...
		for(Document d : documents) {
			...
			Binary binaryByteArray = (Binary) d.get("pdf");
			byte[] byteArray =  binaryByteArray.getData();
	...
	public void store(WarrantyDeed wd) {
		...
		Document doc = new Document("id", wd.getID());
	...
	public void store1(WarrantyDeed wd) {
		...
		Document doc = new Document("text", wd.getText());
		List<String> grantees = Arrays.asList(wd.getGrantees());
		List<String> grantors = Arrays.asList(wd.getGrantors());
